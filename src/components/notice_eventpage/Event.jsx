...
const Event = () => {
    const mainCateNo = 'event'; // 이벤트 카테고리 번호
    const { fetchArticlesByCategory, eventArticles, isLoading, viewUpdate } = useContext(ArticleContext);
    ...
    // 조회수 증가 후 상세보기 이동
    const handleEventClick = async (articleNo) => {
                    ...
                    {eventArticles.length > 0 ? (
                        eventArticles.map((item) => (
                            <div
                                ...
                                onClick={() => handleEventClick(item.articleNo)}
                            >
