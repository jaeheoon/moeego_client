...
import MapPaging from './MapPaging';
...
const SearchBar = () => {
  ...
  const { userInfo, loading, changePage, currentPage, totalPages } = useUserInfo();
  ...
  // 지역에 맞는 아이템 필터링
  const filteredItems = userInfo && userInfo.content ? userInfo.content.filter((item) => {
    if (area === "지역" || area === "전국") {
